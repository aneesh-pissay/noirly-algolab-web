...
import { containerWater } from '@/src/core/algorithms/two-pointers/containerWater';
...
export default function ContainerWaterPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('container-water', containerWater);
    visualizerEngine.execute('container-water', {
        ...
        <div className="flex-1 flex overflow-hidden p-4 gap-4 pt-0">
          ...
          <section className="flex-[1.5] flex flex-col gap-4 min-w-0">
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              ...
              {/* Heights as Vertical Bars */}
              <div className="flex-1 flex items-end justify-center gap-3 relative px-4">
                      ...
                      <div
                        className={`w-full rounded-t-lg transition-all duration-500 flex items-end justify-center pb-2 ${
                          isHighlighted
                            ...
                            : 'bg-surface-container-high/40 border border-outline-variant/20'
                        }`}
                        style={{ height: `${barHeight}%`, minHeight: '40px' }}
                      >
                        <span className={`font-code-md text-sm ${isHighlighted ? 'text-on-primary' : 'text-on-surface-variant'}`}>
                          {height}
                        </span>
                      </div>
                ...
                {/* Water area overlay */}
                {leftPointer >= 0 && rightPointer >= 0 && leftPointer < rightPointer && (
                  <div 
                    className="absolute bottom-0 bg-blue-500/20 border-t-2 border-blue-400/50 pointer-events-none"
                    style={{
                      left: `${(leftPointer / heights.length) * 100}%`,
                      right: `${((heights.length - rightPointer - 1) / heights.length) * 100}%`,
                      height: `${(Math.min(heights[leftPointer], heights[rightPointer]) / Math.max(...heights)) * 100}%`
                    }}
                  />
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-tertiary">{variables.currentArea ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Max Area
                    </span>
                    <span className="font-code-md text-secondary">{variables.maxArea ?? 0}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Width
                    </span>
                    <span className="font-code-md text-primary">{variables.width ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Height
                    </span>
                    <span className="font-code-md text-primary">{variables.height ?? '-'}</span>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      ...
                      <span className="text-tertiary">containerWater</span>(heights) &#123;
                    </div>
                    {/* Line 2 */}
                    ...
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">let</span> right = heights.length - <span className="text-tertiary">1</span>;
                    </div>
                    {/* Line 4 */}
                    <div className={currentStep.codeLine === 4 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">let</span> maxArea = <span className="text-tertiary">0</span>;
                    </div>
                    {/* Line 5 */}
                    <div>
                    ...
                    <div className={currentStep.codeLine === 6 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">const</span> width = right - left;
                    </div>
                    {/* Line 7 */}
                    <div className={currentStep.codeLine === 7 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">const</span> height = Math.<span className="text-tertiary">min</span>(heights[left], heights[right]);
                    </div>
                    {/* Line 8 */}
                    <div className={currentStep.codeLine === 8 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">const</span> area = width * height;
                    </div>
                    {/* Line 9 */}
                    <div className={currentStep.codeLine === 9 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;maxArea = Math.<span className="text-tertiary">max</span>(maxArea, area);
                    </div>
                    {/* Line 10 */}
                    <div className={currentStep.codeLine === 10 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (heights[left] &lt; heights[right]) left++;
                    </div>
                    {/* Line 11 */}
                    <div className={currentStep.codeLine === 11 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">else</span> right--;
                    </div>
                    {/* Line 12 */}
                    <div>
                      &nbsp;&nbsp;&#125;
                    ...
                    <div className={currentStep.codeLine === 13 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">return</span> maxArea;
                    </div>
                    {/* Line 14 */}
                    <div>
                      &#125;
