...
import { combSort } from '@/src/core/algorithms/sorting/combSort';
...
export default function CombSortPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('comb-sort', combSort);
    visualizerEngine.execute('comb-sort', { array: [8, 4, 1, 56, 3, -44, 23, -6, 28, 0] });
  ...
  const gap = currentStep.visualizationData?.gap || variables.gap;
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                <h2 className="font-headline-sm text-headline-sm flex items-center gap-2">
                  ...
                  Array Visualization {gap !== undefined && <span className="text-sm text-tertiary">(Gap: {gap.toFixed(0)})</span>}
                </h2>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10 col-span-2">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Gap</span>
                    <span className="font-code-md text-tertiary text-lg">{gap !== undefined ? gap.toFixed(0) : '-'}</span>
                  </div>
                  ...
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Swapped</span>
                    <span className="font-code-md text-secondary">{variables.swapped ? 'Yes' : 'No'}</span>
                  </div>
                </div>
            ...
            <div className="glass-panel rounded-xl flex-1 flex flex-col overflow-hidden">
              <div className="flex bg-surface-dim/80 border-b border-outline-variant/20">
                <button className="px-4 py-2.5 border-b-2 border-primary bg-surface-container-high text-on-surface text-xs font-label-sm flex items-center gap-2">
                  <span className="w-3 h-3 bg-yellow-400 rounded-sm"></span>CombSort.js
                </button>
              </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      <span className="text-primary-container">function</span> <span className="text-tertiary">combSort</span>(arr) &#123;
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">let</span> gap = arr.length;</div>
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">const</span> shrink = <span className="text-tertiary">1.3</span>;
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">let</span> swapped = <span className="text-tertiary">true</span>;</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">while</span> (gap &gt; <span className="text-tertiary">1</span> || swapped) &#123;</div>
                    <div className={currentStep.codeLine === 6 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;gap = Math.floor(gap / shrink);
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (gap &lt; <span className="text-tertiary">1</span>) gap = <span className="text-tertiary">1</span>;</div>
                    ...
                    <div className={currentStep.codeLine === 9 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = <span className="text-tertiary">0</span>; i + gap &lt; arr.length; i++) &#123;
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (arr[i] &gt; arr[i + gap]) &#123;</div>
