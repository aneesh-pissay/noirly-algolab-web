...
import { minSizeSubarray } from '@/src/core/algorithms/sliding-window/minSizeSubarray';
import { AlgorithmStep, VisualizerState } from '@/src/core/engine/types';

export default function MinSizeSubarrayPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('min-size-subarray', minSizeSubarray);
    visualizerEngine.execute('min-size-subarray', {
      array: [2, 3, 1, 2, 4, 3],
      target: 7
    });
  ...
  const currentSum = currentStep.visualizationData?.currentSum ?? 0;
  const targetSum = currentStep.visualizationData?.targetSum ?? 0;
  const minLength = currentStep.visualizationData?.minLength ?? Infinity;
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              ...
              <div className="flex items-center justify-center gap-6 mb-6">
                <div className="bg-surface-dim p-4 rounded-lg border border-outline-variant/20">
                  <span className="text-xs text-on-surface-variant block mb-1">Current Sum</span>
                  <span className="font-display text-2xl text-primary">{currentSum}</span>
                </div>
                <div className="bg-surface-dim p-4 rounded-lg border border-outline-variant/20">
                  <span className="text-xs text-on-surface-variant block mb-1">Target Sum</span>
                  <span className="font-display text-2xl text-tertiary">{targetSum}</span>
                </div>
                <div className="bg-surface-dim p-4 rounded-lg border border-outline-variant/20">
                  <span className="text-xs text-on-surface-variant block mb-1">Min Length</span>
                  <span className="font-display text-2xl text-green-400">
                    {minLength === Infinity ? '∞' : minLength}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-tertiary">{variables.targetSum ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Window Start
                  ...
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-primary">{variables.currentSum ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Min Length
                    ...
                    <span className="font-code-md text-green-400">
                      {variables.minLength === Infinity ? '∞' : variables.minLength ?? '-'}
                    </span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                ...
                <div className="space-y-2 mt-4 pt-4 border-t border-outline-variant/10">
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant font-body-md">Window Size</span>
                    <span className="text-on-surface font-code-md">
                      {windowStart !== -1 && windowEnd !== -1 ? windowEnd - windowStart + 1 : 0}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      ...
                      <span className="text-tertiary">minSizeSubarray</span>(arr, target) &#123;
                    </div>
                    <div className={currentStep.codeLine === 2 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">let</span> minLength = <span className="text-tertiary">Infinity</span>, windowStart = <span className="text-tertiary">0</span>;
                    </div>
                    ...
                    <div className={currentStep.codeLine === 4 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> windowEnd = <span className="text-tertiary">0</span>; windowEnd &lt; arr.length; windowEnd++) &#123;
                    </div>
                    ...
                    <div className={currentStep.codeLine === 6 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">while</span> (currentSum &gt;= target) &#123;
                    </div>
                    <div className={currentStep.codeLine === 7 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;minLength = Math.min(minLength, windowEnd - windowStart + <span className="text-tertiary">1</span>);
                    </div>
                    ...
                    <div className={currentStep.codeLine === 11 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">return</span> minLength === <span className="text-tertiary">Infinity</span> ? <span className="text-tertiary">0</span> : minLength;
                    </div>
                    <div>
                      &#125;
                    </div>
