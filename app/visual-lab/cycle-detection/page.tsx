...
import { cycleDetection } from '@/src/core/algorithms/two-pointers/cycleDetection';
...
export default function CycleDetectionPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('cycle-detection', cycleDetection);
    visualizerEngine.execute('cycle-detection', {
      nodes: [
        { value: 1, next: 1 },
        { value: 2, next: 2 },
        { value: 3, next: 3 },
  ...
  const nodes = currentStep.visualizationData?.nodes || [];
  const slow = currentStep.visualizationData?.slow ?? -1;
  const fast = currentStep.visualizationData?.fast ?? -1;
  const cycleDetected = currentStep.visualizationData?.cycleDetected || false;
  const meetingPoint = currentStep.visualizationData?.meetingPoint ?? -1;
                  ...
                  return (
                    <div key={index} className="flex items-center gap-6">
                      ...
                      >
                        <div
                          className={`w-20 h-20 rounded-full flex items-center justify-center relative ${
                            isMeetingPoint
                              ? 'bg-error-container/30 border-2 border-error node-glow animate-pulse'
                              : isHighlighted
                        ...
                        >
                          ...
                          {isSlow && (
                            <div className="absolute -top-10 left-1/2 -translate-x-1/2 flex flex-col items-center">
                              <span className="font-code-md text-blue-400 text-sm font-bold mb-1">slow</span>
                              <span
                          ...
                          {isFast && (
                            <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center">
                              ...
                              <span className="font-code-md text-tertiary text-sm font-bold mt-1">fast</span>
                            </div>
                          )}
                        </div>
                      ...
                      {node.next !== null && (
                        ...
                        >
                          {node.next < index ? 'refresh' : 'arrow_forward'}
                        </span>
                      )}
                    </div>
                  );
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-primary">{variables.slow ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Fast Pointer
                    </span>
                    <span className="font-code-md text-primary">{variables.fast ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Cycle Detected
                    ...
                    <span className="font-code-md text-tertiary">
                      {variables.cycleDetected ? 'true' : 'false'}
                    </span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                    ...
                    <span className="font-code-md text-tertiary">
                      {variables.meetingPoint ?? '-'}
                    </span>
                  </div>
                </div>
                <div className="space-y-2 mt-4 pt-4 border-t border-outline-variant/10">
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant font-body-md">List Length</span>
                    <span className="text-on-surface font-code-md">{nodes.length}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className={currentStep.codeLine === 5 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">while</span> (fast && fast.next) &#123;
                    </div>
                    {/* Line 6 */}
                    ...
                    <div className={currentStep.codeLine === 9 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (slow === fast) <span className="text-primary-container">return</span> <span className="text-tertiary">true</span>;
                    </div>
                    {/* Line 10 */}
                    <div>
                      &nbsp;&nbsp;&#125;
