...
import { bstSearch } from '@/src/core/algorithms/trees/bstSearch';
...
export default function BSTSearchPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('bst-search', bstSearch);
    visualizerEngine.execute('bst-search', {
      tree: sampleBST,
      target: 40
    });
    ...
    return (
      <g key={`${node.value}-${x}-${y}`}>
        ...
        <circle
          ...
          cy={y}
          r="28"
          fill={isFound ? '#10b981' : isHighlighted || isCurrent ? '#6366f1' : '#374151'}
          stroke={isFound ? '#059669' : isHighlighted || isCurrent ? '#4f46e5' : '#4a5568'}
          strokeWidth="3"
          className="transition-all duration-300"
          style={{
            filter: isHighlighted || isCurrent ? 'drop-shadow(0 0 12px rgba(99, 102, 241, 0.6))' : 'none'
          }}
        />
            ...
            <div className="glass-panel rounded-xl p-5 flex-1 flex flex-col">
              ...
              <div className="space-y-3">
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  ...
                  <span className="font-code-md text-primary">{variables.currentValue ?? '-'}</span>
                </div>
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                    Search Path
            ...
            <div className="glass-panel rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <span className="font-code-md text-primary">O(h)</span>
                </div>
                <div>
              ...
              <p className="text-xs text-on-surface-variant mt-2">
                h = height of tree (log n average, n worst)
              </p>
            </div>
          </section>
