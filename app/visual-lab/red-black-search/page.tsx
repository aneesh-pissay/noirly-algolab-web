...
import { redBlackSearch } from '@/src/core/algorithms/trees/redBlackSearch';
import { AlgorithmStep, VisualizerState, TreeNode } from '@/src/core/engine/types';

// Sample Red-Black Tree for demonstration
const sampleRBT: TreeNode = {
  value: 40,
  left: {
    value: 20,
    left: { value: 10 },
...
export default function RedBlackSearchPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('red-black-search', redBlackSearch);
    visualizerEngine.execute('red-black-search', {
      tree: sampleRBT,
      target: 50
    });
    ...
    return (
      <g key={`${node.value}-${x}-${y}`}>
        ...
        <circle
          ...
          cy={y}
          r="28"
          fill={isFound ? '#10b981' : isHighlighted || isCurrent ? '#ef4444' : '#374151'}
          stroke={isFound ? '#059669' : isHighlighted || isCurrent ? '#dc2626' : '#4a5568'}
          strokeWidth="3"
          className="transition-all duration-300"
          style={{
            filter: isHighlighted || isCurrent ? 'drop-shadow(0 0 12px rgba(239, 68, 68, 0.6))' : 'none'
          }}
        />
      ...
      <main className="app-main h-screen overflow-hidden flex flex-col">
        ...
        <div className="flex-1 flex overflow-hidden p-4 gap-4 pt-0">
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-2 bg-surface-dim/50 px-4 py-2 rounded-lg border border-outline-variant/10">
                  ...
                  <span className="font-label-sm text-on-surface">Target: <span className="font-code-md text-red-500">{target}</span></span>
                </div>
              </div>
              ...
              <div className="mt-auto pt-6 border-t border-outline-variant/10 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="w-12 h-12 flex items-center justify-center rounded-lg bg-red-600 text-white hover:brightness-110 transition-all shadow-lg shadow-red-600/20"
                  >
          ...
          <section className="flex-1 flex flex-col gap-4 min-w-[320px]">
            {/* Operation Status */}
            <div className="glass-panel rounded-xl p-5 border-l-4 border-l-red-600">
            ...
            <div className="glass-panel rounded-xl p-5 flex-1 flex flex-col">
              ...
              <div className="space-y-3">
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  ...
                  <span className="font-code-md text-red-500">{variables.currentValue ?? '-'}</span>
                </div>
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                    Black Height
                  </span>
                  <span className="font-code-md text-red-500">{variables.blackHeight ?? 0}</span>
                </div>
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                    Comparisons
                  </span>
                  <span className="font-code-md text-red-500">{variables.comparisons ?? 0}</span>
                </div>
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                    Search Path
            ...
            <div className="glass-panel rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <span className="font-code-md text-red-500">O(log n)</span>
                </div>
                <div>
                  <span className="text-xs text-on-surface-variant block mb-1">Space</span>
                  <span className="font-code-md text-red-500">O(1)</span>
                </div>
              </div>
              <p className="text-xs text-on-surface-variant mt-2">
                Guaranteed O(log n) with red-black properties
              </p>
            </div>
          </section>
          ...
          <section className="flex-1 flex flex-col gap-4 min-w-[320px]">
            <div className="glass-panel rounded-xl p-5 flex-1 flex flex-col overflow-hidden">
              ...
              <pre className="flex-1 overflow-auto text-xs font-code-md text-on-surface-variant leading-relaxed">
                <code>{`function redBlackSearch(root, target) {
  let current = root;
  let blackHeight = 0;
  let comparisons = 0;
  ...
  while (current !== null) {
    ...
    // Track black height for visualization
...
// Time: O(log n) - guaranteed by RB properties
