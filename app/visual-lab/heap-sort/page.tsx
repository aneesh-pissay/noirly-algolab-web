...
export default function HeapSortPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('heap-sort', heapSort);
    visualizerEngine.execute('heap-sort', { array: [12, 11, 13, 5, 6, 7] });
                ...
                {arrayData.map((value: number, index: number) => {
                  ...
                  return (
                    <div key={index} className={`flex flex-col items-center gap-4 transition-all duration-500 ${isHighlighted ? 'transform scale-110' : 'opacity-40'}`}>
                      <div className={`w-20 h-24 rounded-xl flex items-center justify-center relative ${
                        ...
                        <span className={`font-display text-display ${isHighlighted ? 'text-primary' : 'text-on-surface'}`}>{value}</span>
                      </div>
                      <span className="font-code-md text-label-sm text-on-surface-variant">[{index}]</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-surface-dim p-2 rounded-lg border border-outline-variant/10">
                    <span className="text-[9px] font-label-sm text-on-surface-variant uppercase block mb-1">n</span>
                    <span className="font-code-md text-sm text-primary">{variables.n ?? arrayData.length}</span>
                  </div>
                  <div className="bg-surface-dim p-2 rounded-lg border border-outline-variant/10">
                    <span className="text-[9px] font-label-sm text-on-surface-variant uppercase block mb-1">i</span>
                    <span className="font-code-md text-sm text-primary">{variables.i ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-2 rounded-lg border border-outline-variant/10">
                    <span className="text-[9px] font-label-sm text-on-surface-variant uppercase block mb-1">largest</span>
                    <span className="font-code-md text-sm text-tertiary">{variables.largest ?? '-'}</span>
                  </div>
                </div>
            ...
            <div className="bg-surface-container-highest/40 rounded-xl p-5 border border-outline-variant/20">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <span className="font-code-md text-lg">O(n log n)</span>
                </div>
                <div className="flex-1">
            ...
            <div className="glass-panel rounded-xl flex-1 flex flex-col overflow-hidden">
              <div className="flex bg-surface-dim/80 border-b border-outline-variant/20">
                <button className="px-4 py-2.5 border-b-2 border-primary bg-surface-container-high text-on-surface text-xs font-label-sm flex items-center gap-2">
                  <span className="w-3 h-3 bg-yellow-400 rounded-sm"></span>HeapSort.js
                </button>
              </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      <span className="text-primary-container">function</span> <span className="text-tertiary">heapSort</span>(arr) &#123;
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">const</span> n = arr.length;</div>
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = Math.floor(n / <span className="text-tertiary">2</span>) - <span className="text-tertiary">1</span>; i &gt;= <span className="text-tertiary">0</span>; i--)
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;heapify(arr, n, i);</div>
                    <div className={currentStep.codeLine === 5 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = n - <span className="text-tertiary">1</span>; i &gt; <span className="text-tertiary">0</span>; i--) &#123;
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;[arr[<span className="text-tertiary">0</span>], arr[i]] = [arr[i], arr[<span className="text-tertiary">0</span>]];</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;heapify(arr, i, <span className="text-tertiary">0</span>);</div>
                    <div>&nbsp;&nbsp;&#125;</div>
                    <div>&#125;</div>
                    <div className={currentStep.codeLine === 10 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>&nbsp;</div>
                    <div><span className="text-primary-container">function</span> <span className="text-tertiary">heapify</span>(arr, n, i) &#123;</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">let</span> largest = i;</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">const</span> left = <span className="text-tertiary">2</span> * i + <span className="text-tertiary">1</span>, right = <span className="text-tertiary">2</span> * i + <span className="text-tertiary">2</span>;</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">if</span> (left &lt; n &amp;&amp; arr[left] &gt; arr[largest]) largest = left;</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">if</span> (right &lt; n &amp;&amp; arr[right] &gt; arr[largest]) largest = right;</div>
                  </div>
                </div>
              </div>
