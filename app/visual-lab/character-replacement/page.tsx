...
import { characterReplacement } from '@/src/core/algorithms/sliding-window/characterReplacement';
import { AlgorithmStep, VisualizerState } from '@/src/core/engine/types';

export default function CharacterReplacementPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('character-replacement', characterReplacement);
    visualizerEngine.execute('character-replacement', {
      str: 'AABABBA',
      k: 1
    });
  ...
  const maxCount = currentStep.visualizationData?.maxCount ?? 0;
  const k = currentStep.visualizationData?.k ?? 0;
          ...
          <section className="flex-[1.5] flex flex-col gap-4 min-w-0">
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              ...
              <div className="flex items-center justify-center gap-6 mb-6">
                ...
                <div className="bg-surface-dim p-4 rounded-lg border border-outline-variant/20">
                  <span className="text-xs text-on-surface-variant block mb-1">Max Count</span>
                  <span className="font-display text-2xl text-primary">{maxCount}</span>
                </div>
                <div className="bg-surface-dim p-4 rounded-lg border border-outline-variant/20">
                  <span className="text-xs text-on-surface-variant block mb-1">K Replacements</span>
                  <span className="font-display text-2xl text-tertiary">{k}</span>
                </div>
                <div className="bg-surface-dim p-4 rounded-lg border border-outline-variant/20">
                  <span className="text-xs text-on-surface-variant block mb-1">Char Frequency</span>
                  <span className="font-code-md text-xs text-primary">
                    {JSON.stringify(charCount)}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-tertiary">{variables.k ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Window Start
                  ...
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-primary">{variables.maxCount ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Max Length
                  ...
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-primary">{variables.replacementsNeeded ?? '-'}</span>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      ...
                      <span className="text-tertiary">characterReplacement</span>(str, k) &#123;
                    </div>
                    <div className={currentStep.codeLine === 2 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">let</span> windowStart = <span className="text-tertiary">0</span>, maxCount = <span className="text-tertiary">0</span>, maxLength = <span className="text-tertiary">0</span>;
                    </div>
                    ...
                    <div className={currentStep.codeLine === 8 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> ((windowEnd - windowStart + <span className="text-tertiary">1</span>) - maxCount &gt; k) &#123;
                    </div>
                    ...
                    <div className={currentStep.codeLine === 10 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;charCount[leftChar]--; windowStart++;
                    </div>
                    <div>
                      &nbsp;&nbsp;&nbsp;&nbsp;&#125;
                    </div>
