...
export default function BucketSortPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('bucket-sort', bucketSort);
    visualizerEngine.execute('bucket-sort', { array: [0.78, 0.17, 0.39, 0.26, 0.72, 0.94, 0.21, 0.12, 0.23, 0.68] });
  ...
  const buckets = currentStep.visualizationData?.buckets;
                  ...
                  return (
                    <div key={index} className={`flex flex-col items-center gap-3 transition-all duration-500 ${isHighlighted ? 'transform scale-110' : 'opacity-40'}`}>
                      <div className={`w-14 h-16 rounded-lg flex items-center justify-center relative ${
                        ...
                        <span className={`font-code-md text-xs ${isHighlighted ? 'text-primary' : 'text-on-surface'}`}>{value.toFixed(2)}</span>
                      </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10 col-span-2">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Bucket Count</span>
                    <span className="font-code-md text-tertiary text-lg">{variables.bucketCount ?? 10}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Current Index</span>
                    <span className="font-code-md text-primary">{variables.i ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Bucket Index</span>
                    <span className="font-code-md text-secondary">{variables.bucketIdx ?? '-'}</span>
                  </div>
                </div>
                <div className="space-y-2 pt-4 border-t border-outline-variant/10">
                  ...
                  {buckets && (
                    <div className="flex flex-col gap-1">
                      <span className="text-on-surface-variant font-body-md text-xs">Buckets</span>
                      <div className="text-on-surface font-code-md text-[10px] max-h-24 overflow-y-auto">
                        {buckets.map((bucket: number[], idx: number) => (
                          <div key={idx} className="py-1">
                            B{idx}: [{bucket.map(v => v.toFixed(2)).join(', ')}]
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className="glass-panel rounded-xl flex-1 flex flex-col overflow-hidden">
              <div className="flex bg-surface-dim/80 border-b border-outline-variant/20">
                <button className="px-4 py-2.5 border-b-2 border-primary bg-surface-container-high text-on-surface text-xs font-label-sm flex items-center gap-2">
                  <span className="w-3 h-3 bg-yellow-400 rounded-sm"></span>BucketSort.js
                </button>
              </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      <span className="text-primary-container">function</span> <span className="text-tertiary">bucketSort</span>(arr, bucketSize = <span className="text-tertiary">5</span>) &#123;
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">if</span> (arr.length === <span className="text-tertiary">0</span>) <span className="text-primary-container">return</span> arr;</div>
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-outline-variant/60">// Create buckets</span>
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">const</span> min = Math.min(...arr);</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">const</span> max = Math.max(...arr);</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">const</span> bucketCount = Math.floor((max - min) / bucketSize) + <span className="text-tertiary">1</span>;</div>
                    <div className={currentStep.codeLine === 7 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">const</span> buckets = Array.from(&#123; length: bucketCount &#125;, () =&gt; []);
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-outline-variant/60">// Distribute into buckets</span></div>
                    ...
                    <div className={currentStep.codeLine === 10 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">const</span> bucketIdx = Math.floor((arr[i] - min) / bucketSize);
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;buckets[bucketIdx].push(arr[i]);</div>
                    <div>&nbsp;&nbsp;&#125;</div>
                    <div>&nbsp;&nbsp;<span className="text-outline-variant/60">// Sort buckets and concatenate</span></div>
                    <div className={currentStep.codeLine === 14 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">return</span> buckets.flatMap(bucket =&gt; bucket.sort((a, b) =&gt; a - b));
                    </div>
                  </div>
                </div>
              </div>
