...
import { moveZeroes } from '@/src/core/algorithms/two-pointers/moveZeroes';
...
export default function MoveZeroesPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('move-zeroes', moveZeroes);
    visualizerEngine.execute('move-zeroes', {
  ...
  const writePointer = currentStep.visualizationData?.writePointer ?? -1;
  const readPointer = currentStep.visualizationData?.readPointer ?? -1;
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              ...
              <div className="flex-1 flex items-center justify-center gap-6 relative">
                {arrayData.map((value: number, index: number) => {
                  ...
                  const showWrite = writePointer === index;
                  const showRead = readPointer === index;
                    ...
                    >
                      ...
                      >
                        ...
                        {showWrite && (
                          ...
                          >
                            <span className="font-code-md text-blue-400 text-sm font-bold mb-1">write</span>
                            <span
                        ...
                        {showRead && (
                          ...
                          >
                            <span className="font-code-md text-tertiary text-sm font-bold mb-1">read</span>
                            <span
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-primary">{variables.writePointer ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Read Pointer
                    </span>
                    <span className="font-code-md text-primary">{variables.readPointer ?? '-'}</span>
                  </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      ...
                      <span className="text-tertiary">moveZeroes</span>(nums) &#123;
                    </div>
                    {/* Line 2 */}
                    <div className={currentStep.codeLine === 2 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">let</span> writePointer = <span className="text-tertiary">0</span>;
                    </div>
                    {/* Line 3 */}
                    <div>
                      &nbsp;
                    ...
                    <div className={currentStep.codeLine === 4 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> readPointer = <span className="text-tertiary">0</span>; readPointer &lt; nums.length; readPointer++) &#123;
                    </div>
                    {/* Line 5 */}
                    <div className={currentStep.codeLine === 5 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (nums[readPointer] !== <span className="text-tertiary">0</span>) &#123;
                    </div>
                    {/* Line 6 */}
