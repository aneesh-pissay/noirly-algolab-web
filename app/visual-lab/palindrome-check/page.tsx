...
import { palindromeCheck } from '@/src/core/algorithms/two-pointers/palindromeCheck';
import { AlgorithmStep, VisualizerState } from '@/src/core/engine/types';

export default function PalindromeCheckPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('palindrome-check', palindromeCheck);
    visualizerEngine.execute('palindrome-check', {
      customInput: 'racecar'
    });
                    ...
                    >
                      <div
                        className={`w-16 h-20 rounded-xl flex items-center justify-center relative ${
                          isHighlighted
                            ? 'bg-purple-500/20 border-2 border-purple-500 node-glow'
                            : 'bg-surface-container-high border border-outline-variant/20'
                        }`}
                      >
                        <span
                          className={`font-display text-2xl ${
                            isHighlighted ? 'text-purple-400' : 'text-on-surface'
                          }`}
                        >
                          {value}
                        </span>
                        {showLeft && (
                          <div className={`absolute -top-10 flex flex-col items-center ${
                            ...
                            <span className="font-code-md text-purple-400 text-sm font-bold mb-1">left</span>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Left</span>
                    <span className="font-code-md text-primary">{variables.left ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Right</span>
                ...
                <div className="space-y-2 mt-4 pt-4 border-t border-outline-variant/10">
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant font-body-md">Original</span>
                    <span className="text-on-surface font-code-md text-xs truncate ml-2">{variables.original || '-'}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant font-body-md">Processed</span>
          ...
          <section className="flex-1 flex flex-col min-w-[380px]">
            <div className="glass-panel rounded-xl flex-1 flex flex-col overflow-hidden">
              <div className="flex bg-surface-dim/80 border-b border-outline-variant/20">
                <button className="px-4 py-2.5 border-b-2 border-purple-500 bg-surface-container-high text-on-surface text-xs font-label-sm flex items-center gap-2">
              ...
              <div className="flex-1 font-code-md text-[13px] leading-6 p-4 overflow-auto bg-surface-dim/30">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div><span className="text-primary-container">function</span> <span className="text-tertiary">isPalindrome</span>(str) &#123;</div>
                    <div>&nbsp;&nbsp;str = str.toLowerCase().replace(/[^a-z0-9]/g, <span className="text-green-400">''</span>);</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">let</span> left = <span className="text-tertiary">0</span>, right = str.length - <span className="text-tertiary">1</span>;</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">while</span> (left &lt; right) &#123;</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (str[left] !== str[right])</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">return</span> <span className="text-tertiary">false</span>;</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;left++; right--;</div>
                    <div>&nbsp;&nbsp;&#125;</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">return</span> <span className="text-tertiary">true</span>;</div>
                    <div>&#125;</div>
                  </div>
                </div>
              </div>
