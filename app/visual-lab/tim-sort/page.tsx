...
import { timSort } from '@/src/core/algorithms/sorting/timSort';
...
export default function TimSortPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('tim-sort', timSort);
    visualizerEngine.execute('tim-sort', { array: [5, 21, 7, 23, 19, 10, 3, 12, 1, 15] });
  ...
  const runSize = variables.runSize || 32;
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                <h2 className="font-headline-sm text-headline-sm flex items-center gap-2">
                  ...
                  Array Visualization <span className="text-sm text-tertiary">(Run Size: {runSize})</span>
                </h2>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10 col-span-2">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Run Size</span>
                    <span className="font-code-md text-tertiary text-lg">{runSize}</span>
                  </div>
            ...
            <div className="bg-surface-container-highest/40 rounded-xl p-5 border border-outline-variant/20">
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <span className="font-code-md text-sm">O(n)</span>
                </div>
              </div>
            </div>
          </section>
            ...
            <div className="glass-panel rounded-xl flex-1 flex flex-col overflow-hidden">
              <div className="flex bg-surface-dim/80 border-b border-outline-variant/20">
                <button className="px-4 py-2.5 border-b-2 border-primary bg-surface-container-high text-on-surface text-xs font-label-sm flex items-center gap-2">
                  <span className="w-3 h-3 bg-yellow-400 rounded-sm"></span>TimSort.js
                </button>
              </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      <span className="text-primary-container">function</span> <span className="text-tertiary">timSort</span>(arr) &#123;
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">const</span> RUN = <span className="text-tertiary">32</span>;</div>
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">const</span> n = arr.length;
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-outline-variant/60">// Sort individual runs</span></div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> start = <span className="text-tertiary">0</span>; start &lt; n; start += RUN) &#123;</div>
                    <div className={currentStep.codeLine === 6 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">const</span> end = Math.min(start + RUN - <span className="text-tertiary">1</span>, n - <span className="text-tertiary">1</span>);
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;insertionSort(arr, start, end);</div>
                    <div>&nbsp;&nbsp;&#125;</div>
                    <div className={currentStep.codeLine === 9 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>&nbsp;&nbsp;<span className="text-outline-variant/60">// Merge runs</span></div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">let</span> size = RUN;</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">while</span> (size &lt; n) &#123;</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> start = <span className="text-tertiary">0</span>; start &lt; n; start += size * <span className="text-tertiary">2</span>) &#123;</div>
                    <div className={currentStep.codeLine === 13 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">const</span> mid = start + size - <span className="text-tertiary">1</span>;
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">const</span> end = Math.min(start + size * <span className="text-tertiary">2</span> - <span className="text-tertiary">1</span>, n - <span className="text-tertiary">1</span>);</div>
