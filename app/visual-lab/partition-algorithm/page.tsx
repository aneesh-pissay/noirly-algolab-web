...
import { partitionAlgorithm } from '@/src/core/algorithms/two-pointers/partitionAlgorithm';
import { AlgorithmStep, VisualizerState } from '@/src/core/engine/types';

export default function PartitionAlgorithmPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('partition-algorithm', partitionAlgorithm);
    visualizerEngine.execute('partition-algorithm', {
  ...
  const pivot = currentStep.visualizationData?.pivot ?? 0;
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-primary/20 rounded-lg border border-primary/30">
                    <span className="text-xs font-label-sm text-on-surface-variant">Pivot:</span>
                    <span className="font-code-md text-primary font-bold">{pivot}</span>
                  </div>
              ...
              <div className="flex-1 flex items-center justify-center gap-6 relative">
                {arrayData.map((value: number, index: number) => {
                  ...
                  const isLessThanPivot = value < pivot;
                  const isGreaterOrEqual = value >= pivot;
                        ...
                        className={`w-20 h-24 rounded-xl flex items-center justify-center relative ${
                          isHighlighted
                            ? isLessThanPivot
                              ...
                              : isGreaterOrEqual
                              ? 'bg-red-500/20 border-2 border-red-500 node-glow'
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-tertiary">{variables.pivot ?? pivot}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Swaps
                    </span>
                    <span className="font-code-md text-secondary">{variables.swaps ?? 0}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Left Partition
                    </span>
                    <span className="font-code-md text-green-400">{variables.leftPartition ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Right Partition
                    </span>
                    <span className="font-code-md text-red-400">{variables.rightPartition ?? '-'}</span>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      ...
                      <span className="text-tertiary">partition</span>(arr, pivot) &#123;
                    </div>
                    {/* Line 2 */}
                    ...
                    <div className={currentStep.codeLine === 5 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (arr[left] &lt; pivot) &#123;
                    </div>
                    {/* Line 6 */}
                    ...
                    <div className={currentStep.codeLine === 7 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&#125; <span className="text-primary-container">else if</span> (arr[right] &gt;= pivot) &#123;
                    </div>
                    {/* Line 8 */}
