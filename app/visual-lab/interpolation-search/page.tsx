...
import { interpolationSearch } from '@/src/core/algorithms/searching/interpolationSearch';
...
export default function InterpolationSearchPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('interpolation-search', interpolationSearch);
    visualizerEngine.execute('interpolation-search', {
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              ...
              <div className="flex-1 flex items-center justify-center gap-3 relative overflow-x-auto px-4">
                {arrayData.map((value: number, index: number) => {
                  ...
                  const showPos = variables.pos === index;
                      ...
                      >
                        ...
                        {showLow && (
                          <div className="absolute -bottom-12 flex flex-col items-center left-1/2 -translate-x-1/2">
                            ...
                            <span className="font-code-md text-secondary text-sm font-bold">low</span>
                          </div>
                        )}
                        {/* Pointer high */}
                        {showHigh && (
                          <div className="absolute -bottom-12 flex flex-col items-center left-1/2 -translate-x-1/2">
                            ...
                            <span className="font-code-md text-error text-sm font-bold">high</span>
                          </div>
                        )}
                        {/* Pointer pos */}
                        {showPos && (
                          <div className="absolute -top-10 flex flex-col items-center left-1/2 -translate-x-1/2">
                            <span className="font-code-md text-primary text-sm font-bold mb-1">pos</span>
                            <span
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-secondary">{variables.low ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      High Index
                    </span>
                    <span className="font-code-md text-error">{variables.high ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10 col-span-2">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Interpolated Position
                    </span>
                    <span className="font-code-md text-primary">{variables.pos ?? '-'}</span>
                  </div>
                </div>
            ...
            <div className="glass-panel rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <span className="font-code-md text-primary">O(log log n)</span>
                </div>
                <div>
