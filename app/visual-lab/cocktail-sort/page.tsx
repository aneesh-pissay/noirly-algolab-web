...
export default function CocktailSortPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('cocktail-sort', cocktailSort);
    visualizerEngine.execute('cocktail-sort', { array: [5, 1, 4, 2, 8, 0, 2] });
  ...
  const direction = currentStep.visualizationData?.direction || 'forward';
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                <h2 className="font-headline-sm text-headline-sm flex items-center gap-2">
                  ...
                  Array Visualization <span className="text-sm text-tertiary">({direction === 'forward' ? '→' : '←'})</span>
                </h2>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10 col-span-2">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Direction</span>
                    <span className="font-code-md text-tertiary capitalize">{direction}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Start</span>
                    <span className="font-code-md text-primary">{variables.start ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">End</span>
                    <span className="font-code-md text-secondary">{variables.end ?? '-'}</span>
                  </div>
                </div>
                <div className="space-y-2 pt-4 border-t border-outline-variant/10">
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant font-body-md">Swapped</span>
                    <span className="text-on-surface font-code-md">{variables.swapped ? 'Yes' : 'No'}</span>
                  </div>
                  <div className="flex justify-between text-sm">
            ...
            <div className="glass-panel rounded-xl flex-1 flex flex-col overflow-hidden">
              <div className="flex bg-surface-dim/80 border-b border-outline-variant/20">
                <button className="px-4 py-2.5 border-b-2 border-primary bg-surface-container-high text-on-surface text-xs font-label-sm flex items-center gap-2">
                  <span className="w-3 h-3 bg-yellow-400 rounded-sm"></span>CocktailSort.js
                </button>
              </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      <span className="text-primary-container">function</span> <span className="text-tertiary">cocktailSort</span>(arr) &#123;
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">let</span> swapped = <span className="text-tertiary">true</span>;</div>
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">let</span> start = <span className="text-tertiary">0</span>, end = arr.length - <span className="text-tertiary">1</span>;
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">while</span> (swapped) &#123;</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;swapped = <span className="text-tertiary">false</span>;</div>
                    <div className={currentStep.codeLine === 6 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-outline-variant/60">// Forward pass</span>
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = start; i &lt; end; i++) &#123;</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (arr[i] &gt; arr[i + <span className="text-tertiary">1</span>]) &#123;</div>
                    <div className={currentStep.codeLine === 9 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[arr[i], arr[i + <span className="text-tertiary">1</span>]] = [arr[i + <span className="text-tertiary">1</span>], arr[i]];
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;swapped = <span className="text-tertiary">true</span>;</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&#125;</div>
                    <div className={currentStep.codeLine === 12 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&#125;
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-outline-variant/60">// Backward pass</span></div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = end - <span className="text-tertiary">1</span>; i &gt;= start; i--) &#123;</div>
