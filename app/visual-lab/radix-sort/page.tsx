...
export default function RadixSortPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('radix-sort', radixSort);
    visualizerEngine.execute('radix-sort', { array: [170, 45, 75, 90, 802, 24, 2, 66] });
  ...
  const digit = currentStep.visualizationData?.digit;
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                <h2 className="font-headline-sm text-headline-sm flex items-center gap-2">
                  ...
                  Array Visualization {digit && <span className="text-sm text-tertiary">(Digit: {digit === 1 ? 'ones' : digit === 10 ? 'tens' : digit === 100 ? 'hundreds' : digit})</span>}
                </h2>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10 col-span-2">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Current Digit Position</span>
                    <span className="font-code-md text-tertiary text-lg">{variables.exp ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Max Value</span>
                    <span className="font-code-md text-primary">{variables.max ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Digit</span>
                    <span className="font-code-md text-primary">{variables.digit ?? '-'}</span>
                  </div>
                </div>
            ...
            <div className="bg-surface-container-highest/40 rounded-xl p-5 border border-outline-variant/20">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <span className="font-code-md text-sm">O(d(n+k))</span>
                </div>
                <div className="flex-1">
                  <span className="text-[10px] text-on-surface-variant uppercase block">Space</span>
                  <span className="font-code-md text-sm">O(n+k)</span>
                </div>
              </div>
            </div>
          </section>
            ...
            <div className="glass-panel rounded-xl flex-1 flex flex-col overflow-hidden">
              <div className="flex bg-surface-dim/80 border-b border-outline-variant/20">
                <button className="px-4 py-2.5 border-b-2 border-primary bg-surface-container-high text-on-surface text-xs font-label-sm flex items-center gap-2">
                  <span className="w-3 h-3 bg-yellow-400 rounded-sm"></span>RadixSort.js
                </button>
              </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      <span className="text-primary-container">function</span> <span className="text-tertiary">radixSort</span>(arr) &#123;
                    </div>
                    ...
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-outline-variant/60">// Process each digit</span>
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> exp = <span className="text-tertiary">1</span>; Math.floor(max / exp) &gt; <span className="text-tertiary">0</span>; exp *= <span className="text-tertiary">10</span>) &#123;</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;countingSortByDigit(arr, exp);</div>
                    <div>&nbsp;&nbsp;&#125;</div>
                    <div className={currentStep.codeLine === 7 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>&nbsp;</div>
                    <div><span className="text-primary-container">function</span> <span className="text-tertiary">countingSortByDigit</span>(arr, exp) &#123;</div>
                    ...
                    <div className={currentStep.codeLine === 10 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">const</span> count = <span className="text-primary-container">new</span> Array(<span className="text-tertiary">10</span>).fill(<span className="text-tertiary">0</span>);
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-outline-variant/60">// Count occurrences</span></div>
                    ...
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;count[Math.floor(arr[i] / exp) % <span className="text-tertiary">10</span>]++;</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = <span className="text-tertiary">1</span>; i &lt; <span className="text-tertiary">10</span>; i++)</div>
