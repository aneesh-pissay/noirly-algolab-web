...
export default function ShellSortPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('shell-sort', shellSort);
    visualizerEngine.execute('shell-sort', { array: [12, 34, 54, 2, 3] });
  ...
  const gap = currentStep.visualizationData?.gap;
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                <h2 className="font-headline-sm text-headline-sm flex items-center gap-2">
                  ...
                  Array Visualization {gap && <span className="text-sm text-tertiary">(Gap: {gap})</span>}
                </h2>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10 col-span-2">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Gap Size</span>
                    <span className="font-code-md text-tertiary text-lg">{variables.gap ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">i</span>
                    <span className="font-code-md text-primary">{variables.i ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">j</span>
            ...
            <div className="glass-panel rounded-xl flex-1 flex flex-col overflow-hidden">
              <div className="flex bg-surface-dim/80 border-b border-outline-variant/20">
                <button className="px-4 py-2.5 border-b-2 border-primary bg-surface-container-high text-on-surface text-xs font-label-sm flex items-center gap-2">
                  <span className="w-3 h-3 bg-yellow-400 rounded-sm"></span>ShellSort.js
                </button>
              </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      <span className="text-primary-container">function</span> <span className="text-tertiary">shellSort</span>(arr) &#123;
                    </div>
                    ...
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> gap = Math.floor(n / <span className="text-tertiary">2</span>); gap &gt; <span className="text-tertiary">0</span>; gap = Math.floor(gap / <span className="text-tertiary">2</span>)) &#123;
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-outline-variant/60">// Gapped insertion sort</span></div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = gap; i &lt; n; i++) &#123;</div>
                    <div className={currentStep.codeLine === 6 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">const</span> temp = arr[i];
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">let</span> j = i;</div>
                    <div className={currentStep.codeLine === 8 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">while</span> (j &gt;= gap &amp;&amp; arr[j - gap] &gt; temp) &#123;
                    </div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;arr[j] = arr[j - gap];</div>
