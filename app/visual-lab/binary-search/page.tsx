...
import { binarySearch } from '@/src/core/algorithms/searching/binarySearch';
...
export default function BinarySearchPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('binary-search', binarySearch);
    visualizerEngine.execute('binary-search', {
                    ...
                    >
                      ...
                      >
                        ...
                        {showLeft && (
                          <div
                            className={`absolute -top-10 flex flex-col items-center ${
                              showLeft && (showMid || showRight) ? 'left-1/4 -translate-x-1/2' : 'left-1/2 -translate-x-1/2'
                            }`}
                          >
                            <span className="font-code-md text-secondary text-sm font-bold mb-1">left</span>
                        ...
                        {showMid && (
                          <div
                            className={`absolute -top-10 flex flex-col items-center ${
                              showLeft && showMid ? 'left-2/4 -translate-x-1/2' : 'left-1/2 -translate-x-1/2'
                            }`}
                          >
                            <span className="font-code-md text-primary text-sm font-bold mb-1">mid</span>
                        ...
                        {showRight && (
                          <div
                            className={`absolute -top-10 flex flex-col items-center ${
                              showRight && (showLeft || showMid) ? 'left-3/4 -translate-x-1/2' : 'left-1/2 -translate-x-1/2'
                            }`}
                          >
            ...
            <div className="glass-panel rounded-xl p-5 flex-1 flex flex-col">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Mid</span>
                    <span className="font-code-md text-primary">{variables.mid ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Right</span>
                    <span className="font-code-md text-tertiary">{variables.right ?? '-'}</span>
                  </div>
                </div>
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Comparisons</span>
                  <span className="font-code-md text-primary">{variables.comparisonCount ?? 0}</span>
                </div>
                {foundIndex !== undefined && (
                  <div className={`p-3 rounded-lg border-2 ${
                    foundIndex >= 0 ? 'bg-tertiary-container/10 border-tertiary' : 'bg-error-container/10 border-error'
                  }`}>
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Result</span>
