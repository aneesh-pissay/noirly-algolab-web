...
import { countAnagrams } from '@/src/core/algorithms/sliding-window/countAnagrams';
import { AlgorithmStep, VisualizerState } from '@/src/core/engine/types';

export default function CountAnagramsPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('count-anagrams', countAnagrams);
    visualizerEngine.execute('count-anagrams', {
      str: 'cbaebabacd',
      pattern: 'abc'
    });
  ...
  const anagramsFound = currentStep.visualizationData?.anagramsFound ?? 0;
  const anagramIndices = currentStep.visualizationData?.anagramIndices || [];
        ...
        <div className="flex-1 flex overflow-hidden p-4 gap-4 pt-0">
          ...
          <section className="flex-[1.5] flex flex-col gap-4 min-w-0">
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              ...
              {/* Pattern Display */}
              <div className="mb-6 bg-surface-container-high/50 rounded-lg p-4 border border-outline-variant/20">
                <span className="text-xs font-label-sm text-on-surface-variant uppercase tracking-widest block mb-2">
                  Pattern to Find
                </span>
                <div className="flex gap-2">
                  {pattern.split('').map((char: string, idx: number) => (
                    <div
                      key={idx}
                      className="w-12 h-12 rounded-lg bg-tertiary-container/30 border border-tertiary flex items-center justify-center"
                    >
                      <span className="font-code-md text-xl text-tertiary">{char}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex-1 flex items-center justify-center gap-2 relative flex-wrap">
                {stringData.map((char: string, index: number) => {
                  ...
                  const isAnagramStart = anagramIndices.includes(index);
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-14 h-16 rounded-xl flex items-center justify-center relative ${
                          isAnagramStart
                            ? 'bg-tertiary-container/40 border-2 border-tertiary node-glow'
                            : isHighlighted
              ...
              <div className="mt-6 bg-surface-container-high/50 rounded-lg p-4 border border-outline-variant/20">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-display text-3xl text-primary">{anagramsFound}</span>
                </div>
                {anagramIndices.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-outline-variant/10">
                    <span className="text-[10px] text-on-surface-variant uppercase block mb-2">Indices</span>
                    <div className="flex gap-2 flex-wrap">
                      {anagramIndices.map((idx: number, i: number) => (
                        <span key={i} className="px-2 py-1 bg-tertiary-container/20 rounded text-xs font-code-md text-tertiary">
                          {idx}
                        </span>
                      ))}
                    </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-tertiary">{variables.anagramsFound ?? 0}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Found At
                    </span>
                    <span className="font-code-md text-tertiary text-xs">
                      {variables.foundAt ?? '-'}
                    </span>
                  </div>
                </div>
                <div className="space-y-2 mt-4 pt-4 border-t border-outline-variant/10">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-on-surface font-code-md text-xs">"{pattern}"</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      ...
                      <span className="text-tertiary">countAnagrams</span>(str, pattern) &#123;
                    </div>
                    {/* Line 2 */}
                    ...
                    {/* Line 5 */}
                    <div>
                      &nbsp;&nbsp;<span className="text-primary-container">const</span> charFreq = &#123;&#125;;
                    </div>
                    {/* Line 6 */}
                    <div>
                    ...
                    <div className={currentStep.codeLine === 8 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-on-surface-variant">// Update frequency map</span>
                    </div>
                    {/* Line 9 */}
                    <div className={currentStep.codeLine === 9 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (windowEnd - windowStart + <span className="text-tertiary">1</span> === pattern.length) &#123;
                    </div>
                    {/* Line 10 */}
                    <div className={currentStep.codeLine === 10 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (matched === pattern.length) result.push(windowStart);
                    </div>
                    {/* Line 11 */}
                    <div>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;windowStart++;
