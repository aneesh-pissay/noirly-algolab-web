...
import { graphDFS } from '@/src/core/algorithms/graphs/graphDFS';
...
export default function DFSPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('graph-dfs', graphDFS);
    visualizerEngine.execute('graph-dfs', {
      graph: sampleGraph,
      startVertex: 0
    });
  ...
  // Graph layout positions (same as BFS)
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                <h2 className="font-headline-sm text-headline-sm flex items-center gap-2">
                  <span className="material-symbols-outlined text-purple-500">device_hub</span>
                  Depth-First Search
                </h2>
                <div className="flex items-center gap-2 bg-surface-dim/50 px-4 py-2 rounded-lg border border-outline-variant/10">
                  ...
                  <span className="font-label-sm text-on-surface">Recursive Traversal</span>
                </div>
              </div>
                    ...
                    toList.map((to: number) => {
                      ...
                      return (
                        <line
                          ...
                          x2={positions[to].x}
                          y2={positions[to].y}
                          stroke={isExploring ? '#8b5cf6' : '#4a5568'}
                    ...
                    return (
                      <g key={v}>
                        <circle
                          ...
                          cy={positions[v].y}
                          r="25"
                          fill={isCurrent ? '#8b5cf6' : isVisited ? '#10b981' : '#374151'}
                          stroke={isHighlighted ? '#8b5cf6' : '#4a5568'}
                          strokeWidth="3"
                          className="transition-all duration-300"
                          style={{
                            filter: isHighlighted ? 'drop-shadow(0 0 12px rgba(139, 92, 246, 0.6))' : 'none'
                          }}
                        />
                        <text
                          x={positions[v].x}
            ...
            <div className="glass-panel rounded-xl p-5 flex-1 flex flex-col">
              ...
              <div className="space-y-3">
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  ...
                  <span className="font-code-md text-purple-500">{variables.currentVertex ?? '-'}</span>
                </div>
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                    Depth
                  </span>
                  <span className="font-code-md text-purple-500">{variables.depth ?? 0}</span>
                </div>
                <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                  <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                    Visited
            ...
            <div className="glass-panel rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <span className="font-code-md text-purple-500">O(V + E)</span>
                </div>
                <div>
                  <span className="text-xs text-on-surface-variant block mb-1">Space</span>
                  <span className="font-code-md text-purple-500">O(V)</span>
                </div>
              </div>
...
// Space: O(V) - recursion stack and visited set`}</code>
              </pre>
            </div>
          </section>
        </div>
