...
import { trappingRainWater } from '@/src/core/algorithms/two-pointers/trappingRainWater';
...
export default function TrappingRainWaterPage() {
  ...
  useEffect(() => {
    ...
    visualizerEngine.registerAlgorithm('trapping-rain-water', trappingRainWater);
    visualizerEngine.execute('trapping-rain-water', {
  ...
  const heights = currentStep.visualizationData?.heights || [];
  const waterLevels = currentStep.visualizationData?.waterLevels || [];
        ...
        <div className="flex-1 flex overflow-hidden p-4 gap-4 pt-0">
          ...
          <section className="flex-[1.5] flex flex-col gap-4 min-w-0">
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              ...
              {/* Heights with Water Levels */}
              <div className="flex-1 flex items-end justify-center gap-2 relative px-4">
                {heights.map((height: number, index: number) => {
                  const isHighlighted = highlights.includes(index);
                  const showLeft = leftPointer === index;
                  const showRight = rightPointer === index;
                  const maxHeight = Math.max(...heights, ...(waterLevels.length > 0 ? waterLevels : [0]));
                  const barHeight = (height / maxHeight) * 100;
                  const waterLevel = waterLevels[index] || 0;
                  const waterHeight = waterLevel > 0 ? ((waterLevel - height) / maxHeight) * 100 : 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-full flex flex-col items-center relative" style={{ height: `${Math.max(barHeight, waterLevel > 0 ? (waterLevel / maxHeight) * 100 : 0)}%`, minHeight: '40px' }}>
                        {/* Water overlay */}
                        {waterHeight > 0 && (
                          <div 
                            className="w-full bg-blue-400/40 border border-blue-400/60 rounded-t-lg"
                            style={{ height: `${waterHeight}%` }}
                          />
                        )}
                        {/* Ground bar */}
                        <div
                          className={`w-full rounded-t-lg transition-all duration-500 flex items-end justify-center pb-1 mt-auto ${
                            isHighlighted
                              ? 'bg-primary/60 border-2 border-primary node-glow'
                              : 'bg-surface-container-high border border-outline-variant/20'
                          }`}
                          style={{ height: `${barHeight}%`, minHeight: height > 0 ? '30px' : '2px' }}
                        >
                          {height > 0 && (
                            <span className={`font-code-md text-xs ${isHighlighted ? 'text-on-primary' : 'text-on-surface-variant'}`}>
                              {height}
                            </span>
                          )}
                        </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    ...
                    <span className="font-code-md text-blue-400">{variables.leftMax ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Right Max
                    </span>
                    <span className="font-code-md text-tertiary">{variables.rightMax ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Total Water
                    </span>
                    <span className="font-code-md text-secondary">{variables.totalWater ?? 0}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">
                      Water Added
                    </span>
                    <span className="font-code-md text-blue-400">{variables.waterAdded ?? 0}</span>
                  </div>
                </div>
                <div className="space-y-2 mt-4 pt-4 border-t border-outline-variant/10">
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant font-body-md">Heights Count</span>
                    <span className="text-on-surface font-code-md">{heights.length}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant font-body-md">Heights</span>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      ...
                      <span className="text-tertiary">trappingRainWater</span>(heights) &#123;
                    </div>
                    {/* Line 2 */}
                    <div className={currentStep.codeLine === 2 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">let</span> left = <span className="text-tertiary">0</span>, right = heights.length - <span className="text-tertiary">1</span>;
                    </div>
                    {/* Line 3 */}
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">let</span> leftMax = <span className="text-tertiary">0</span>, rightMax = <span className="text-tertiary">0</span>;
                    </div>
                    {/* Line 4 */}
                    <div className={currentStep.codeLine === 4 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">let</span> totalWater = <span className="text-tertiary">0</span>;
                    </div>
                    {/* Line 5 */}
                    <div>
                    ...
                    <div className={currentStep.codeLine === 6 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (heights[left] &lt; heights[right]) &#123;
                    </div>
                    {/* Line 7 */}
                    <div className={currentStep.codeLine === 7 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (heights[left] &gt;= leftMax) leftMax = heights[left];
                    </div>
                    {/* Line 8 */}
                    <div className={currentStep.codeLine === 8 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">else</span> totalWater += leftMax - heights[left];
                    </div>
                    {/* Line 9 */}
                    ...
                    {/* Line 10 */}
                    <div>
                      &nbsp;&nbsp;&nbsp;&nbsp;&#125; <span className="text-primary-container">else</span> &#123;
                    </div>
                    {/* Line 11 */}
                    <div className={currentStep.codeLine === 11 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">if</span> (heights[right] &gt;= rightMax) rightMax = heights[right];
                    </div>
                    {/* Line 12 */}
                    <div className={currentStep.codeLine === 12 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-primary-container">else</span> totalWater += rightMax - heights[right];
                    </div>
                    {/* Line 13 */}
                    ...
                    <div className={currentStep.codeLine === 16 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">return</span> totalWater;
                    </div>
                    {/* Line 17 */}
                    <div>
                      &#125;
