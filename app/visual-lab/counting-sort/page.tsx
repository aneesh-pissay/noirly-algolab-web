...
import { countingSort } from '@/src/core/algorithms/sorting/countingSort';
...
export default function CountingSortPage() {
  ...
  useEffect(() => {
    visualizerEngine.registerAlgorithm('counting-sort', countingSort);
    visualizerEngine.execute('counting-sort', { array: [4, 2, 2, 8, 3, 3, 1] });
  ...
  const countArray = currentStep.visualizationData?.countArray;
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col flex-1 relative overflow-hidden">
              ...
              <div className="flex-1 flex items-center justify-center gap-4 relative">
                {arrayData.map((value: number, index: number) => {
                  ...
                  return (
                    <div key={index} className={`flex flex-col items-center gap-4 transition-all duration-500 ${isHighlighted ? 'transform scale-110' : 'opacity-40'}`}>
                      <div className={`w-16 h-20 rounded-xl flex items-center justify-center relative ${
                        ...
                        <span className={`font-display text-xl ${isHighlighted ? 'text-primary' : 'text-on-surface'}`}>{value}</span>
                      </div>
                      <span className="font-code-md text-[10px] text-on-surface-variant">[{index}]</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Min</span>
                    <span className="font-code-md text-primary">{variables.min ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Max</span>
                    <span className="font-code-md text-secondary">{variables.max ?? '-'}</span>
                  </div>
                  <div className="bg-surface-dim p-3 rounded-lg border border-outline-variant/10 col-span-2">
                    <span className="text-[10px] font-label-sm text-on-surface-variant uppercase block mb-1">Range</span>
                    <span className="font-code-md text-tertiary">{variables.range ?? '-'}</span>
                  </div>
                </div>
                <div className="space-y-2 pt-4 border-t border-outline-variant/10">
                  ...
                  {countArray && (
                    <div className="flex justify-between text-sm">
                      <span className="text-on-surface-variant font-body-md">Count Array</span>
                      <span className="text-on-surface font-code-md text-xs">[{countArray.join(', ')}]</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm">
            ...
            <div className="bg-surface-container-highest/40 rounded-xl p-5 border border-outline-variant/20">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <span className="font-code-md text-lg">O(n + k)</span>
                </div>
                <div className="flex-1">
                  <span className="text-[10px] text-on-surface-variant uppercase block">Space</span>
                  <span className="font-code-md text-lg">O(k)</span>
                </div>
              </div>
            </div>
          </section>
            ...
            <div className="glass-panel rounded-xl flex-1 flex flex-col overflow-hidden">
              <div className="flex bg-surface-dim/80 border-b border-outline-variant/20">
                <button className="px-4 py-2.5 border-b-2 border-primary bg-surface-container-high text-on-surface text-xs font-label-sm flex items-center gap-2">
                  <span className="w-3 h-3 bg-yellow-400 rounded-sm"></span>CountingSort.js
                </button>
              </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className={currentStep.codeLine === 1 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      <span className="text-primary-container">function</span> <span className="text-tertiary">countingSort</span>(arr) &#123;
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">const</span> max = Math.max(...arr);</div>
                    <div className={currentStep.codeLine === 3 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-primary-container">const</span> min = Math.min(...arr);
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">const</span> count = <span className="text-primary-container">new</span> Array(max - min + <span className="text-tertiary">1</span>).fill(<span className="text-tertiary">0</span>);</div>
                    <div className={currentStep.codeLine === 5 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-outline-variant/60">// Count occurrences</span>
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = <span className="text-tertiary">0</span>; i &lt; arr.length; i++)</div>
                    <div>&nbsp;&nbsp;&nbsp;&nbsp;count[arr[i] - min]++;</div>
                    <div className={currentStep.codeLine === 8 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;<span className="text-outline-variant/60">// Cumulative sum</span>
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = <span className="text-tertiary">1</span>; i &lt; count.length; i++)</div>
                    <div className={currentStep.codeLine === 10 ? 'bg-primary/10 -mx-4 px-4 border-l-2 border-primary' : ''}>
                      &nbsp;&nbsp;&nbsp;&nbsp;count[i] += count[i - <span className="text-tertiary">1</span>];
                    </div>
                    <div>&nbsp;&nbsp;<span className="text-outline-variant/60">// Build output</span></div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">const</span> output = <span className="text-primary-container">new</span> Array(arr.length);</div>
                    <div>&nbsp;&nbsp;<span className="text-primary-container">for</span> (<span className="text-primary-container">let</span> i = arr.length - <span className="text-tertiary">1</span>; i &gt;= <span className="text-tertiary">0</span>; i--) &#123;</div>
